...
//        scanner4();
    ...
    private static void objectInputOutputStream() {
        System.out.println("\nobjectInputOutputStream():");
        String fileName = "someClass2.bin";
        try{
        ...
        try (
        		//Usamos ObjectOutputStream y FileOutputStream para desserializar un objeto
        		ObjectOutputStream objectOutputStream = new ObjectOutputStream(new FileOutputStream(fileName));
        		ObjectInputStream objectInputStream = new ObjectInputStream(new FileInputStream(fileName))
        	){
            SomeClass obj = new SomeClass();

            //Copiamos el objeto al archivo
            objectOutputStream.writeObject(obj);

            //ahora leemos ese archivo y desserializamos el contenido con objectInputStream
    ...
    //El printStream extiende del FilterOutputStream el cual extiende de OutputStream
    private static void printStream1() {
        System.out.println("\nprintStream1():");
        String fileName = "output.txt";
        try(FileOutputStream  fos = new FileOutputStream(fileName);
            PrintStream ps = new PrintStream(fos)){
            ps.println("Hi there again!");
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    private static void printStream4() {
        System.out.println("\nprintStream4():");
        System.out.printf("Hi, %s! %s", "dear reader", "otra cosa");  //prints: Hi, dear reader!
        System.out.format("%nHi, %s!%n", "dear reader");  //prints: Hi, dear reader!
    }
    ...
    //El writer es el analogo del OutputStream
